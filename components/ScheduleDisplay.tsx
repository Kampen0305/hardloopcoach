...
import { TrainingProgram, WeeklySchedule, Workout } from '../types';
...
const WeekItem: React.FC<{ weekData: WeeklySchedule, initialOpen?: boolean }> = ({ weekData, initialOpen = false }) => {
  ...
  return (
    <div className="mb-6 bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 ease-in-out print-container">
      ...
      {/* For printing, we might want all weeks to be open or styled differently if closed.
          The current @media print CSS doesn't force open, but this could be a future enhancement. */}
...
const ScheduleDisplay: React.FC<ScheduleDisplayProps> = ({ program }) => {
  if (!program) {
    ...
    <section className="mt-10" id="schedule-display-section" aria-label="Generated Schedule">
      ...
      <div className="space-y-6">
        {program.weeklySchedule.map((weekData, index) => (
          <WeekItem key={weekData.week} weekData={weekData} initialOpen={index < 2} /> // Open first two weeks by default
        ))}
      </div>
      ...
      <div id="action-buttons-container" className="mt-10 flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4 no-print">
        ...
        <button
          onClick={handleShare}
          className="px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-colors"
        >
          Schema Delen
        </button>
        <button
          onClick={handlePrint}
          className="px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-slate-700 bg-slate-200 hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 transition-colors"
        >
          Print Schema
        </button>
      </div>
